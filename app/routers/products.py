...
async def all_products(db: Annotated[AsyncSession, Depends(get_db)]):
    products = await db.scalars(select(Product).where(Product.stock > 0, Product.is_active == True))
...
async def create_product(db: Annotated[AsyncSession, Depends(get_db)], create_product: CreateProduct,
    ...
    if get_user.get('is_admin') or get_user.get('is_supplier'):
        category_id = await db.scalar(select(Category.id).where(Category.name == create_product.category))
...
async def product_detail(db: Annotated[AsyncSession, Depends(get_db)], product_slug: str):
    product = await db.scalar(select(Product).where(
        Product.slug == product_slug, Product.stock > 0, Product.is_active == True))
